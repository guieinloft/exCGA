...
float clamp(float a, float l1, float l2)
{
	/* operação branchless que representa o clamp */
	return a * (a >= l1 && a <= l2) + l1 * (a < l1) + l2 * (a > l2);
}
...
int hash_function(glm::vec3 point)
{
	...
	int x = (int)clamp(point.x, 0.0f, (float)SCREEN_W - 1) / scale_x;
	int y = (int)clamp(point.y, 0.0f, (float)SCREEN_H - 1) / scale_y;
...
enum direction hash_direction(glm::vec3 point)
{
	...
	int x_1 = round(clamp(point.x, 0.0f, (float)SCREEN_W - 1) / scale_x);
	int y_1 = round(clamp(point.y, 0.0f, (float)SCREEN_H - 1) / scale_y);
